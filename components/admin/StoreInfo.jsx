    ...
    return (
        <div className="flex-1 space-y-2 text-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
                ...
                {/* Status Badge */}
                <span
                    className={`text-xs font-semibold px-4 py-1 rounded-full ${store.status === 'đang_chờ_xử_lý'
                        ? 'bg-yellow-100 text-yellow-800'
                        : store.status === 'đã_từ_chối'
            ...
            <p className="text-slate-700 mt-5">Đăng ký vào <span className="text-xs">{new Date(store.createdAt).toLocaleDateString()}</span> bởi</p>
